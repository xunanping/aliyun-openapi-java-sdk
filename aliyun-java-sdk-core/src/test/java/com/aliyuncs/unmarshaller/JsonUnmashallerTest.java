...
public class JsonUnmashallerTest {
  ...
  public void unmarshalTest() throws ClientException {
    ...
    try {
      ...
      Assert.assertEquals("SDK.UnmarshalFailed : unmarshal response from json content failed, clazz = DescribeRegionsResponse, origin response = invalid json", ex.getMessage());
      return;
    }
    Assert.fail();
  }
